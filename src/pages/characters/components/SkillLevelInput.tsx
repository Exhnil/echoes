...
const SkillLevelInput = ({ value, onChange }: SkillLevelInputProps) => (
    <div className="flex">
        <div className="flex bg-gray-500 rounded-l-lg overflow-hidden">
            <input
                min={1}
                max={10}
                value={value}
        ...
        <div className="flex flex-col">
            <button
                className="bg-gray-600 rounded-tr-lg w-8 h-6 cursor-pointer"
                onClick={() => value < 10 && onChange(value + 1)}
            >
                +
            </button>
            <button
                className="bg-gray-600 rounded-br-lg w-8 h-6 cursor-pointer"
